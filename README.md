...
- Graph uses SDL2 at runtime (requires SDL2 installed in the system). `Graph:label(...)` and `Graph:button(...)` now render built-in bitmap text without SDL_ttf, `Graph:button("Text", "CommandName")` can call a Brise command on click, and `Graph:render(0)` / `Graph:render("forever")` keeps the window open until the user closes it. `Graph:update(...)` and `Graph:refresh(...)` are aliases for re-rendering with the same SDL loop.
